...
const destinations = ['米沢', '諏訪', '富山', '松本'];
...
export default function Home() {
  ...
  // ユニークなキー作成（年月日）
  const makeKey = (year: number, month0based: number, day: number) =>
    `${year}-${month0based}-${day}`;
  ...
  const handleDeletePlan = async () => {
    ...
    try {
      ...
      console.error('Failed to delete plan in Firestore:', e);
      setModalVisible(false);
    }
  };
  ...
  const renderDayCell = (day: number | null, idx: number) => {
    if (!day)
      return (
        <td key={`empty-${idx}`} style={{ width: 40, height: 60, backgroundColor: '#f9f9f9' }} />
      );
    ...
    const sorted = [...dayPlans].sort(
      (a, b) =>
        family.findIndex((f) => f.name === a.name) - family.findIndex((f) => f.name === b.name)
    );
  ...
  // 選択日・選択人物の予定を取得
  const currentPlan =
    selectedDateKey && plans[selectedDateKey]
      ? plans[selectedDateKey].find((p) => p.name === selectedPerson)
      : null;
        ...
        {family.map((person) => (
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              border: '1px solid #ccc',
              backgroundColor: selectedPerson === person.name ? '#eee' : 'white',
              cursor: 'pointer',
              opacity: selectedPerson === person.name ? 0.6 : 1,
            }}
          >
            <span
              style={{
      ...
      {modalVisible && (
        ...
        >
          ...
          >
            {currentPlan ? (
              <>
                <h3>現在の行先：{currentPlan.destination}</h3>
                <button
                ...
                <h4 style={{ marginTop: 20 }}>行先を変更</h4>
              </>
            ) : (
              <h3>行先を選択</h3>
            )}
